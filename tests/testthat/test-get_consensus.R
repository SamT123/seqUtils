...
test_that("get_consensus fill defaults to ? and is configurable", {
  sequences <- c("ACGT", "ACAT", "ATTT")

  expect_equal(get_consensus(sequences, min_freq = 0.7), "A??T")
  expect_equal(get_consensus(sequences, min_freq = 0.7, fill = "N"), "ANNT")
})

test_that("get_consensus fills columns where every character is excluded", {
  sequences <- c("AC-T", "AC-T")

  expect_equal(get_consensus(sequences, excluded_characters = "-"), "AC?T")
  expect_equal(
    get_consensus(
      sequences,
      excluded_characters = "-",
